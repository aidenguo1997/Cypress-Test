...
declare global {
  namespace Cypress {
    interface Chainable {
      ...
      attemptCreateDepartment(departmentName: string, errorMsg?: string): Chainable<void>;
    }
  }
}
...
Cypress.Commands.add(
  'attemptCreateDepartment',
  (departmentName: string, errorMsg = 'This department already exists. Please enter a new department name.') => {
    cy.get('input[data-testid="new_dept_name"]').type(departmentName);
    cy.get('button[data-testid="ok_btn"]').then(($btn) => {
      if ($btn.is(':disabled')) {
        cy.get('p[class="chakra-text css-0"]')
        ...
        cy.wrap($btn).click();
        cy.get('p[class="chakra-text css-os2hgt"]').should(
          'contain',
          'Your department has been successfully created!'
        );
        ...
        cy.get('td[class="table_cell__HzHwi table_firstRowCell__IFizZ css-xumdn4"]')
          .eq(1)
...
describe('建立新部門', () => {
  it('建立成功或動態重新輸入名稱', () => {
    const generateDepartmentName = (): string =>
      `${testData.departmentName}${Math.floor(Math.random() * 1000)}`;
